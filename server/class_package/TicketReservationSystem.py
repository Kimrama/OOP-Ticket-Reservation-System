...
from .Meal_Reservation import Meal_Reservation
...
class TicketReservationSystem:
    ...
    def delete_a_bogie_from_train(self, train_id, bogie_id) :
        response = self.delete_bogie_from_train(train_id, bogie_id)
        return response
    
    def delete_a_seat_from_bogie(self, train_id, bogie_id, seat_id) :
        response = self.delete_seat_from_bogie(train_id, bogie_id, seat_id)
        return response
    
    def change_coupon_discount(self, coupon_id, new_discount) :
        response = self.update_coupon_discount(coupon_id, new_discount)
        return response
    
    def change_meal_price(self, meal_id, new_price) :
        response = self.update_meal_price(meal_id, new_price)
        return response
    
    def delete_a_coupon_from_system(self, coupon_id) :
        response = self.delete_coupon_from_system(coupon_id)
        return response
    
    def add_a_coupon_to_system(self, coupon_id, discount) :
        response = self.create_coupon_to_system(coupon_id, discount)
        return response
    
    def add_a_bogie_to_train(self, train_id) :
        response = self.create_bogie_to_train(train_id)
        return response
    
    def add_seat_to_bogie(self, train_id, bogie_id, amount) :
        response = self.create_seat_to_bogie(train_id, bogie_id, amount)
        return response
    ...
    def search_meal_by_meal_id(self, meal_id) :
        for meal in self.__meal_list :
            if meal.get_meal_id() == meal_id : return meal
        else : return None
    ...
    def create_coupon_to_system(self, coupon_id, discount) :
        for coupon in self.__coupon_list :
            if coupon.get_coupon_id() == coupon_id : return {"message": "error this coupon is aleardy in the system."}
        self.__coupon_list.append(Coupon(coupon_id, discount))
        return {"message": "done with adding a coupon"}
    
    def create_bogie_to_train(self, train_id) :
        train = self.search_train_from_train_id(train_id)
        train.add_bogie(Bogie())
        return {"status": f"done with adding a bogie"}
        
    def create_seat_to_bogie(self, train_id, bogie_id, amount) :
        bogie = self.search_bogie_from_bogie_id_and_train_id(train_id, bogie_id)
        if bogie == None : return {"message": "not found"}
        enter_amount = amount
        while amount > 0 :
            bogie.add_seat(Seat())
            amount -= 1
        return {"status": f"done with adding {enter_amount} seats"}
    ...
    #! destucture
    def delete_bogie_from_train(self, train_id, bogie_id) :
        train = self.search_train_from_train_id(train_id)
        if train == None : return {"message": "fail train not found"}
        bogie = train.get_bogie_by_bogie_id(bogie_id)
        if bogie == None : return {"message": "fail bogie not found"}
        
        
        if bogie.delete_all_seat() == "done" and train.delete_bogie(bogie) == "done" :
            return {"message": "done with delete the bogie"}
        else : return {"message": "fail something wrong"}
    
    def delete_seat_from_bogie(self, train_id, bogie_id, seat_id) :
        bogie = self.search_bogie_from_bogie_id_and_train_id(train_id, bogie_id)
        if bogie == None : return {"message": "fail bogie not found"}
        if bogie.delete_seat(seat_id) == "done" : return {"message": "done with delete the seat"}
        else : return {"message": "fail seat not found"}
    
    def delete_coupon_from_system(self, coupon_id):
        coupon = self.search_coupon_by_coupon_id(coupon_id)
        if coupon == None: return {"message": "fail coupon not found"}
        self.__coupon_list.remove(coupon)
        del coupon
        return {"message": "done with delete the coupon"}
    #!updating
    
    def update_coupon_discount(self, coupon_id, new_discount) :
        coupon = self.search_coupon_by_coupon_id(coupon_id)
        if coupon == None : return {"mesaage" : "fail coupon not found"}
        coupon.update_discount(new_discount)
        return {"message": "done with updating new discount"}
    
    def update_meal_price(self, meal_id, new_price) :
        meal = self.search_meal_by_meal_id(meal_id)
        if meal == None : return {"mesaage" : "fail meal not found"}
        meal.update_price(new_price)
        return {"message": "done with updating new price"}